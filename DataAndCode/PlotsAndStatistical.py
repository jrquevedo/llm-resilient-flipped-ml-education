...
#%% Practical Activities and Final Examan Assessments
...
for year in years: 
    ...
    [nstudents,pa_arrays,finalTook]=read_grades('PAandFinalExaAssessments{}.csv'.format(year))
    nPA=len(pa_arrays)
...
group_a = finalTooks[0]
group_b = finalTooks[1]

print('\nFinal Exam grades:')
print('2024-2025 N={} Mean={:5.2f} Variance={:5.4f}'.format(len(group_a),np.mean(group_a),np.var(group_a)))
print('2025-2026 N={} Mean={:5.2f} Variance={:5.4f}'.format(len(group_b),np.mean(group_b),np.var(group_b)))

# 2. Ejecutamos el Welch's t-test
# Nota: equal_var=False es lo que activa el t-test de Welch
t_stat, p_valor = ttest_ind(group_a, group_b, equal_var=False)
...
df = get_welch_df(group_a, group_b)
...
#%% Theory exam grades for comparison

def read_theory_grades(csv_file):
    """
    Reads a CSV file with one column of theory grades and returns
    a numpy array with the numeric values.
    """
...
notas24_25 = read_theory_grades('final_theory_exam_grades_2025.csv')
notas25_26 = read_theory_grades('final_theory_exam_grades_2026.csv')
